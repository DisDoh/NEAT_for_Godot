...
func _init(number_inputs: int,
	...
	Params.arr_hidden_layer = array_hidden_layer
...
func create_initial_population() -> Array:
	...
	var hidden_layers = {}; var temp_hidden_layers = {}
	...
	# generate all hidden layers neurons
	for i in range (Params.arr_hidden_layer.size()):
		for j in range (Params.arr_hidden_layer[i]):
			# calculate the position of the input or bias neuron (in the first layer)
			var new_pos = Vector2(0, float(i)/Params.arr_hidden_layer[i])
			# the first neuron should be the bias neuron
			var neuron_type = Params.NEURON_TYPE.hidden
#			if not made_bias:
#				neuron_type = Params.NEURON_TYPE.bias
#				made_bias = true
			# register neuron in Innovations, make the new neuron
			...
			var new_neuron = Neuron.new(initial_neuron_id,
										...
										Params.default_curve,
										false)
			temp_hidden_layers[initial_neuron_id] = new_neuron
		hidden_layers[i] = temp_hidden_layers
		temp_hidden_layers = {}
	...
	var temp_all_neurons 
	var all_neurons 
	temp_all_neurons = Utils.merge_dicts(input_neurons, hidden_layers[0])
	for i in range(hidden_layers.size() - 1):
		temp_all_neurons = Utils.merge_dicts(temp_all_neurons, hidden_layers[i + 1])

	all_neurons = Utils.merge_dicts(temp_all_neurons, output_neurons)
	...
	for _initial_genome in Params.population_size:
		...
		while links_added < Params.num_initial_links:
			...
			for i in range(hidden_layers.size() - 1):
				var from_hidden_neuron_id = Utils.random_choice(hidden_layers[i].keys())
				var to_hidden_neuron_id = Utils.random_choice(hidden_layers[i + 1].keys())
				...
				if neurons[from_neuron_id].neuron_type != Params.NEURON_TYPE.bias:
					...
					var innov_id = Innovations.check_new_link(from_hidden_neuron_id, to_hidden_neuron_id)
