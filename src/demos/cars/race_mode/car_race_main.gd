...
#	var track_path = "res://demos/cars/tracks/track_%s/Track_%s.tscn" % [track_num, track_num]
#	var track = load(track_path).instance()
#	add_child(track)
	load_track(curr_track_num)
	...
	if curr_track_num == 3:
		$Track/FullLap/CheckpointCollider10.disabled = true
		$Track/HalfLap/CheckpointCollider7.disabled = true
	...
	cam = load("res://NEAT_usability/camera/ZoomPanCam.tscn").instance()
	add_child(cam)
	cam.position = $Track/Start.position
	cam.zoom *= 0.3
	cam.make_current()
	# start the race
	paused = false
...
func add_opponents_to_track(opponent_names: Array) -> void:
	...
	for opponent_name in opponent_names:
		...
		var genome: Genome
		genome = Genome.new(index, new_opponent.network.neurons, new_opponent.network.links)
		
		"""Create a new genome detail window (shows connections of the network).
		"""
		var GenomeDetail = load("res://NEAT_usability/gui/genome_detail/GenomeDetail.tscn")

		var new_genome_detail = GenomeDetail.instance()
		new_genome_detail.inspected_genome = genome
		new_genome_detail.inspected_genome.agent = genome.set_agent(new_opponent.network)
		new_genome_detail.inspected_genome.agent.set_network(new_opponent.network)
		$Track/Start.add_child(new_genome_detail)
#		new_genome_detail.rect_position = new_opponent.car.global_position

		$Track/Start.add_child(new_opponent.car)
		index += 1

#	var gui = load("res://NEAT_usability/gui/NeatGUI.gd").new()
#	add_child(gui)
